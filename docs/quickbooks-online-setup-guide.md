...
| *"This commitment has no Contract Company…"* (or *"…no Owner/Client…"*) | QBO requires a vendor on every Bill/PO and a customer on every Invoice; the SiteCommand record's company field is blank | Edit the record → set **Contract Company** (commitments) or **Owner/Client** (prime contracts) → sync again |
| *"Could not resolve or create QBO vendor/customer '<name>': …"* | The lookup/create call to QBO failed; the reason after the colon is QBO's own error (often auth/environment related) | Act on the quoted QBO reason — e.g. reconnect if it's an auth fault, or fix the environment selector if keys don't match |
