      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-white shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Email threads linked to this project</p>
          </div>
        ...
        {/* Connection banner */}
        {connection !== null && (
          <div className={`px-6 py-3 border-b text-sm flex items-center justify-between shrink-0 ${
            connection.connected
            ...
            {connection.connected ? (
              <>
                <span>
                  Connected as <strong>{connection.displayName || connection.email}</strong> ({connection.email})
                </span>
                <button
                  onClick={disconnect}
                  className="text-xs underline hover:text-green-900 ml-4"
                >
                  Disconnect
                </button>
              </>
                  ...
                  {threads.map((t) => (
                    <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3">
                        ...
                        <div className="text-xs text-gray-400 truncate max-w-xs mt-0.5">{t.latest_message_preview}</div>
                      </td>
                      <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="relative ml-auto w-[480px] bg-white shadow-xl flex flex-col h-full">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 shrink-0">
              <h2 className="text-sm font-semibold text-gray-900">Link Email Thread</h2>
              <button onClick={() => setShowLinkPanel(false)} className="text-gray-400 hover:text-gray-600">
